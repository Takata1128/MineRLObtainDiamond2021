...
if [[ " $@ " =~ " --no-data " ]]; then
   AICROWD_DATA_ENABLED="NO"
    ...
    /opt/conda/envs/minerl/bin/python ./utility/verify_or_download_data.py
fi
...
eval "/opt/conda/envs/minerl/bin/python run.py --seeds 1 $EXTRAOUTPUT &"
trap "kill -11 $! > /dev/null 2>&1;" EXIT
...
eval "/opt/conda/envs/minerl/bin/python run.py $EXTRAOUTPUT && touch $EXITED_SIGNAL_PATH || touch $EXITED_SIGNAL_PATH &"
trap "kill -11 $! > /dev/null 2>&1;" EXIT
